...
public:
        ...
        , LangOpts(LangOpts)
    {
        ...
        Out << common::index::getUSRSpacePrefix();
    }
    ...
    void VisitObjCContainerDecl(
        const ObjCContainerDecl *CD, const ObjCCategoryDecl *CatD = nullptr);
    ...
    {
        common::index::generateUSRForObjCClass(
            cls, Out, ExtSymDefinedIn, CategoryContextExtSymbolDefinedIn);
    }

    /// Generate a USR for an Objective-C class category.
    void GenObjCCategory(
        StringRef cls, StringRef cat, StringRef clsExt, StringRef catExt)
    {
        common::index::generateUSRForObjCCategory(cls, cat, Out, clsExt, catExt);
    }

    /// Generate a USR fragment for an Objective-C property.
    void GenObjCProperty(StringRef prop, bool isClassProp)
    {
        common::index::generateUSRForObjCProperty(prop, isClassProp, Out);
    }

    /// Generate a USR for an Objective-C protocol.
    void GenObjCProtocol(StringRef prot, StringRef ext)
    {
        common::index::generateUSRForObjCProtocol(prot, Out, ext);
    }
...
void USRGenerator::VisitFunctionDecl(const FunctionDecl *D)
{
    ...
    // Forward references can have different template argument names. Suppress the template argument names in constructors to make their USR more stable.
...
void USRGenerator::VisitObjCMethodDecl(const ObjCMethodDecl *D)
{
    ...
    if (const ObjCProtocolDecl *pd = dyn_cast<ObjCProtocolDecl>(container)) {
        ...
        // The USR for a method declared in a class extension or category is based on the ObjCInterfaceDecl, not the ObjCCategoryDecl.
...
void USRGenerator::VisitObjCContainerDecl(
    const ObjCContainerDecl *D, const ObjCCategoryDecl *CatD)
{
    switch (D->getKind()) {
    default:
    ...
    case Decl::ObjCCategory: {
        ...
        if (CD->IsClassExtension()) {
            ...
            GenObjCCategory(ID->getName(), CD->getName(),
                GetExternalSourceContainer(ID), GetExternalSourceContainer(CD));
    ...
    if (EmitDeclName(D)) {
        if (const TypedefNameDecl *TD = D->getTypedefNameForAnonDecl()) {
            ...
            if (D->isEmbeddedInDeclarator() && !D->isFreeStanding()) {
                printLoc(
                    Out, D->getLocation(), Context->getSourceManager(), true);
            }
            else {
                Buf[off] = 'a';
...
bool USRGenerator::GenLoc(const Decl *D, bool IncludeOffset)
{
    ...
    IgnoreResults = IgnoreResults ||
        printLoc(
            Out, D->getBeginLoc(), Context->getSourceManager(), IncludeOffset);
...
#include "clang/AST/BuiltinTypes.def"
            case BuiltinType::Dependent:
                // If you're adding a new builtin type, please add its name
                // prefixed with "@BT@" to `Out` (see cases above).
                IgnoreResults = true;
                break;
            }
            return;
...
//            T.dump();
        ...
        if (const FunctionProtoType *FT = T->getAs<FunctionProtoType>()) {
            ...
            clang::RefQualifierKind rq = FT->getRefQualifier();
...
static void combineClassAndCategoryExtContainers(
    StringRef ClsSymDefinedIn, StringRef CatSymDefinedIn, raw_ostream &OS)
{
...
void common::index::generateUSRForObjCClass(StringRef Cls, raw_ostream &OS,
    StringRef ExtSymDefinedIn, StringRef CategoryContextExtSymbolDefinedIn)
{
    combineClassAndCategoryExtContainers(
        ExtSymDefinedIn, CategoryContextExtSymbolDefinedIn, OS);
    OS << "objc(cs)" << Cls;
}

void common::index::generateUSRForObjCCategory(StringRef Cls, StringRef Cat,
    raw_ostream &OS, StringRef ClsSymDefinedIn, StringRef CatSymDefinedIn)
{
    combineClassAndCategoryExtContainers(ClsSymDefinedIn, CatSymDefinedIn, OS);
    OS << "objc(cy)" << Cls << '@' << Cat;
}

void common::index::generateUSRForObjCIvar(StringRef Ivar, raw_ostream &OS)
{
    OS << '@' << Ivar;
}

void common::index::generateUSRForObjCMethod(
    StringRef Sel, bool IsInstanceMethod, raw_ostream &OS)
{
    OS << (IsInstanceMethod ? "(im)" : "(cm)") << Sel;
}

void common::index::generateUSRForObjCProperty(
    StringRef Prop, bool isClassProp, raw_ostream &OS)
{
    OS << (isClassProp ? "(cpy)" : "(py)") << Prop;
}

void common::index::generateUSRForObjCProtocol(
    StringRef Prot, raw_ostream &OS, StringRef ExtSymDefinedIn)
{
...
void common::index::generateUSRForGlobalEnum(
    StringRef EnumName, raw_ostream &OS, StringRef ExtSymDefinedIn)
{
    if (!ExtSymDefinedIn.empty())
        OS << "@M@" << ExtSymDefinedIn;
    OS << "@E@" << EnumName;
...
void common::index::generateUSRForEnumConstant(
    StringRef EnumConstantName, raw_ostream &OS)
{
    OS << '@' << EnumConstantName;
}

bool common::index::generateUSRForDecl(const Decl *D, SmallVectorImpl<char> &Buf)
{
    if (!D)
        return true;
...
bool common::index::generateUSRForDecl(
    const Decl *D, SmallVectorImpl<char> &Buf, const LangOptions &LangOpts)
{
    if (!D)
        return true;
...
bool common::index::generateUSRForMacro(const MacroDefinitionRecord *MD,
    const SourceManager &SM, SmallVectorImpl<char> &Buf)
{
    if (!MD)
        return true;
    return generateUSRForMacro(
        MD->getName()->getName(), MD->getLocation(), SM, Buf);
}

bool common::index::generateUSRForMacro(StringRef MacroName, SourceLocation Loc,
    const SourceManager &SM, SmallVectorImpl<char> &Buf)
{
    if (MacroName.empty())
        return true;
...
bool common::index::generateUSRForType(
    QualType T, ASTContext &Ctx, SmallVectorImpl<char> &Buf)
{
    return generateUSRForType(T, Ctx, Buf, Ctx.getLangOpts());
}

bool common::index::generateUSRForType(QualType T, ASTContext &Ctx,
    SmallVectorImpl<char> &Buf, const LangOptions &LangOpts)
{
    if (T.isNull())
        return true;
...
bool common::index::generateFullUSRForModule(const Module *Mod, raw_ostream &OS)
{
    if (!Mod->Parent)
...
bool common::index::generateFullUSRForTopLevelModuleName(
    StringRef ModName, raw_ostream &OS)
{
    OS << getUSRSpacePrefix();
    return generateUSRFragmentForModuleName(ModName, OS);
}

bool common::index::generateUSRFragmentForModule(
    const Module *Mod, raw_ostream &OS)
{
    return generateUSRFragmentForModuleName(Mod->Name, OS);
}

bool common::index::generateUSRFragmentForModuleName(
    StringRef ModName, raw_ostream &OS)
{
    OS << "@M@" << ModName;
    return false;

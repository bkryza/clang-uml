...
//
// void set_module(graph_t &j, const common::model::element &e)
//{
...
/*
void to_json(graph_t &j, const class_element &c)
{
    j["name"] = c.name();
...
void to_json(graph_t &j, const class_member &c)
{
    j = dynamic_cast<const class_element &>(c);
    j["is_static"] = c.is_static();
}

void to_json(graph_t &j, const method_parameter &c)
{
    j["name"] = c.name();
    j["type"] = c.type();
...
void to_json(graph_t &j, const class_method &c)
{
    j = dynamic_cast<const class_element &>(c);
...
void to_json(graph_t &j, const objc_method &c)
{
    j = dynamic_cast<const class_element &>(c);
...
void to_json(graph_t &j, const class_ &c)
{
...
void to_json(graph_t &j, const objc_interface &c)
{
    j = dynamic_cast<const common::model::element &>(c);
...
void to_json(graph_t &j, const enum_ &c)
{
    j = dynamic_cast<const common::model::element &>(c);
    j["is_nested"] = c.is_nested();
...
void to_json(graph_t &j, const concept_ &c)
{
    j = dynamic_cast<const common::model::element &>(c);
...
void generator::generate_diagram(graph_t &parent) const
{
    /*
...
void generator::generate_top_level_elements(graph_t &parent) const
{
...
void generator::generate(const package &p, graph_t &parent) const
{
...
void generator::generate(const class_ &c, graph_t &parent) const
{
    clanguml::common::generators::graphml::vertex_t class_node;
    class_node.id = c.alias();
    class_node.type = "class";
    class_node.name = c.full_name(true);

    auto maybe_link_pattern = get_link_pattern(c);
    if (maybe_link_pattern) {
        const auto &[link_prefix, link_pattern] = *maybe_link_pattern;
        auto ec = element_context(c);
        common::generators::make_context_source_relative(ec, link_prefix);

        class_node.url = env().render(std::string_view{link_pattern}, ec);
    }

    parent.add_vertex(c.id().value(), std::move(class_node));
    ...
    //            tp["type"] =
    //            config().using_namespace().relative(tp.at("type"));
    //        }
    //    }
...
void generator::generate(const enum_ &e, graph_t &parent) const
{
    clanguml::common::generators::graphml::vertex_t enum_node;
    enum_node.id = e.alias();
    enum_node.type = "enum";
    enum_node.name = e.full_name(true);
    ...
    if (maybe_link_pattern) {
        ...
        enum_node.url = env().render(std::string_view{link_pattern}, context);
    }

    parent.add_vertex(e.id().value(), std::move(enum_node));

    //    nlohmann::json object = e;
    //
    //    if (!config().generate_fully_qualified_name())
    //        object["display_name"] =
    //            common::generators::graphml::render_name(e.full_name_no_ns());
    //
    //    parent["elements"].push_back(std::move(object));
}

void generator::generate(const concept_ &c, graph_t &parent) const
{
    //    nlohmann::json object = c;
    //
    //    if (!config().generate_fully_qualified_name())
...
void generator::generate(const objc_interface &c, graph_t &parent) const
{
    //    nlohmann::json object = c;
    //
    //    // Perform config dependent postprocessing on generated class
...
void generator::generate_relationships(graph_t &parent) const
{
    for (const auto &p : model()) {
        if (auto *pkg = dynamic_cast<package *>(p.get()); pkg) {
